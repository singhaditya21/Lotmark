...
class Manager extends EventEmitter {
    ...
    async #trackJobsFailed(name, jobs, err) {
        ...
        for (const job of jobs) {
            ...
            if (persisted?.state === 'failed') {
                spy.addJob(job.id, name, job.data, 'failed', persisted.output ?? { message: err?.message, stack: err?.stack });
            }
    ...
    #trackJobsSettled(name, completed, failed) {
        ...
        for (const { job, output } of failed) {
            spy.addJob(job.id, name, job.data, 'failed', stringify(output));
        }
    }
    ...
    async work(name, ...args) {
        ...
        const resolveInterval = (lastFetchCount) => {
            ...
            const burst = fullBatch && ((burstWhenReadyExceeds !== undefined && getReadyCount() > burstWhenReadyExceeds) ||
        ...
        const createWorker = (workerId, workId) => {
            ...
            return new Worker({ id: workerId, workId, name, options, resolveInterval, fetch, onFetch, onError });
        };
    ...
    async getQueueStats(name, options = {}) {
        ...
        const toSnapshot = (row) => {
            ...
            for (const field of STATS_COUNT_FIELDS) {
                ...
                if (value !== undefined && value !== null)
                    snapshot[field] = isCockroach ? Number(value) : value;
            }
            return snapshot;
        };
        if (this.config.persistQueueStats) {
            ...
            assert(Number.isInteger(limit) && limit >= 1 && limit <= 100_000, 'getQueueStats: limit must be an integer between 1 and 100000');
            ...
            if (bucketSeconds !== undefined || maxDataPoints !== undefined) {
                assert(aggregate === 'max' || aggregate === 'min' || aggregate === 'avg', "getQueueStats: aggregate must be 'max', 'min', or 'avg'");
                const mode = bucketSeconds !== undefined ? 'bucket' : 'auto';
                const width = bucketSeconds ?? maxDataPoints;
                assert(Number.isInteger(width) && width >= 1, `getQueueStats: ${mode === 'bucket' ? 'bucketSeconds' : 'maxDataPoints'} must be a positive integer`);
        ...
        const maxCacheAgeMs = (options.force
            ...
            : Math.max(QUEUE_STATS_CACHE_TTL_SECONDS, this.config.monitorIntervalSeconds ?? 0, this.config.superviseIntervalSeconds ?? 0)) * 1000;
