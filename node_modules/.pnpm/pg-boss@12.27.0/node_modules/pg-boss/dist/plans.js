...
import { normalizeSchemaName, resolveSchemaName } from "./tools.js";
...
export function createQueue(schema, name, options, noAdvisoryLocks) {
...
export function notifyChannelSql(schema) {
...
export function notifyQueue(schema, name) {
    return `SELECT pg_notify(${notifyChannelSql(schema)}, '${name}')`;
}
export function deleteQueue(schema, name, noAdvisoryLocks) {
...
export function getSchedulesByQueueAndKey(schema) {
...
export function ensureQueueStatsPartitions(schema) {
    return `
    DO $$
    DECLARE
      d date;
...
export function dropOldQueueStatsPartitions(schema, days) {
    return `
    DO $$
    DECLARE
      r record;
...
export function deleteOldQueueStats(schema, days) {
    return `
    DELETE FROM ${schema}.queue_stats
    WHERE captured_on < now() - interval '${days} days'
  `;
}
export function insertQueueStats(schema, queues, noAdvisoryLocks) {
    const sql = `
...
export function getQueueStatsCache(schema) {
    return `
    SELECT
      name,
...
export function getQueueStatsHistory(schema) {
    return `
    SELECT
      name,
      deferred_count as "deferredCount",
...
export function getSchemaCaseVariants(schema) {
...
export function getPartitionedQueueTables(schema) {
...
export function completeJobsWithOutputs(schema, table) {
    return `
    WITH input AS (
...
export function insertFlowJobs(schema, { table, name }, jobs) {
...
export function failJobsByTimeout(schema, table, queues, noAdvisoryLocks) {
...
export function failJobsByHeartbeat(schema, table, queues, noAdvisoryLocks) {
    const where = `state = '${JOB_STATES.active}'
...
export function failJobsByIdWithOutputs(schema, table) {
...
export function deadLetterJobsByIdWithOutputs(schema, table) {
    const where = `name = $1 AND id IN (SELECT id FROM output_map) AND state < '${JOB_STATES.completed}'`;
    const output = '(SELECT om.output FROM output_map om WHERE om.id = deleted_jobs.id)';
    return `
    WITH output_map AS (
...
export function selectJobsToFailByTimeout(schema, table, queues) {
    return {
...
export function selectJobsToFailByHeartbeat(schema, table, queues) {
    return {
        text: `SELECT * FROM ${schema}.${table}
      WHERE state = '${JOB_STATES.active}'
        AND heartbeat_seconds IS NOT NULL
...
export function updateJob(schema, table, name, by, match, notify = false) {
...
export function cacheQueueStats(schema, table, queues, noAdvisoryLocks) {
...
export function refreshQueueStats(schema, table, name) {
...
export function locked(schema, query, key, noAdvisoryLocks) {
...
export function insertDependencies(schema, deps) {
    const sql = `
...
export function cleanupDependencies(schema, table, queues, noAdvisoryLocks) {
    const sql = `
