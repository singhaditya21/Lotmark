 ...
 */
export type UpdateOptions = Pick<JobOptions, 'id' | 'priority' | 'startAfter' | 'singletonKey' | 'group' | 'deadLetter'> & QueueOptions & ConnectionOptions & {
 ...
 */
export type WorkHandlerFor<O extends WorkOptions, ReqData, ResData = any> = O extends {
    perJobResults: true;
} ? (O extends {
    includeMetadata: true;
} ? PerJobWorkWithMetadataHandler<ReqData> : PerJobWorkHandler<ReqData>) : (O extends {
    includeMetadata: true;
} ? WorkWithMetadataHandler<ReqData, ResData> : WorkHandler<ReqData, ResData>);
