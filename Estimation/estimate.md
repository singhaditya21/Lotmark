...
eleven sheets including live Gantt and resource-loading charts, regenerated by
...
## 12 · Two priced offers

Two versions of Stage 1, each scoped to land exactly on a stated final cost, and
each including one **Claude Max 20x** seat. Workbooks:
[`IPC-Stage-1-Offer-A-20-lakh.xlsx`](IPC-Stage-1-Offer-A-20-lakh.xlsx) and
[`IPC-Stage-1-Offer-B-30-lakh.xlsx`](IPC-Stage-1-Offer-B-30-lakh.xlsx),
generated by [`model/build_variants.py`](model/build_variants.py).

| | **Offer A** | **Offer B** |
|---|---|---|
| Final cost, inc-GST | **₹20,00,000** | **₹30,00,000** |
| Contract value ex-GST | ₹16,94,915 | ₹25,42,373 |
| Effort at P80 | 122 PD | 179 PD |
| Duration | 20 weeks | 24 weeks |
| Work packages | 20 of 25 | 25 of 25 |
| Milestones | 5 | 6 |
| Claude Max 20x | 5 months · ₹95,000 | 6 months · ₹1,14,000 |
| **Supplier margin** | **12.8%** | **15.7%** |

**The method is design-to-cost, and the margin is the tell.** Rates and method
are fixed; scope is cut until it fits the ceiling; the margin is whatever falls
out, and it is printed on the Commercials sheet rather than buried. The first
attempt at ₹20 lakh — the full Stage 1 scope minus a few packages — came out at
**3.1% margin**, which is not a business. Offer A only became viable after five
packages were cut outright and eight resized. Below roughly 10%, the honest
answer is that the ceiling should move rather than the scope.

**What ₹20 lakh gives up.** Offer A drops the DBIM logo determination, CSP
hardening, Wayback orphan recovery, the GIGW policy pages and the destination
status-layer design, and halves the redirect map, the PDF estate work and triage
facilitation. It delivers a remediated live site and one authoritative register,
with **redirects by pattern** and only the traffic-bearing tail decided by hand.
Offer B delivers the row-by-row map, a signed PDF disposition across ~2,758
documents, and the Stage 2 cost baseline as a contracted deliverable. Everything
cut from A reappears in Stage 2, generally dearer — the Scope sheet shows the
two side by side, package by package.

**The licence.** One Claude Max 20x seat at **USD 200/month**, converted at
**₹95/USD**. The tier price comes from secondary sources; `claude.com/pricing`
itself lists Max as "From $100 per month" without separating the 5x and 20x
tiers, so confirm at purchase. It is an imported digital service, so 18% IGST
applies under reverse charge and is recoverable as input credit by a registered
supplier — hence it sits ex-tax in the cost base. **No productivity multiplier
has been applied to the unit rates.** There is no evidence base for a specific
one, so the licence is carried as a cost the supplier bears rather than as a
discount to IPC.

Neither offer buys the migration. Stage 2 stays a separate procurement in both.
...
| [`model/variants.json`](model/variants.json) | The two priced offers: ceiling, scope cuts, licence, milestones |
| [`model/build_variants.py`](model/build_variants.py) | Solves scope back from each ceiling and builds both offer workbooks |
