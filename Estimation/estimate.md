...
## 11 · Stage 1 timeline and payment schedule

**24 weeks · 5.5 months · 179 PD at P80 · ₹2,388,000 ex-GST.**
Full workbook: [`Stage-1-Timeline-and-Payments.xlsx`](Stage-1-Timeline-and-Payments.xlsx),
regenerated by [`model/build_schedule.py`](model/build_schedule.py).

```
        W1    W4    W8    W12   W16   W20   W24
Ph 0    ████████████████████                      Mandate and decisions
Ph 1      ██████████████                          Emergency remediation
Ph 2        ████████████████████████████████████  Inventory and redirect map
PMO     ████████████████████████████████████████  Programme management
         ◆M1      ◆M2   ◆M3   ◆M4        ◆M5  ◆M6
```

**Phase 2 starts in week 3, alongside Phase 1 rather than after it.** The
302-to-404 behaviour measured on 1 September is a usable existence oracle, so
the inventory is not blocked on the 404 fix — which takes roughly six weeks out
of the plan's original sequencing.

| Ref | Week | Milestone | Share | Ex-GST | Inc-GST |
|---|---:|---|---:|---:|---:|
| **M1** | W2 | Mobilisation and inception report | 10% | ₹238,800 | ₹281,784 |
| **M2** | W8 | Live site remediated | 20% | ₹477,600 | ₹563,568 |
| **M3** | W12 | Platform decision pack | 15% | ₹358,200 | ₹422,676 |
| **M4** | W16 | Draft inventory: the union register | 20% | ₹477,600 | ₹563,568 |
| **M5** | W20 | Signed disposition register and redirect map | 25% | ₹597,000 | ₹704,460 |
| **M6** | W24 | Stage 2 cost baseline and handover | 10% | ₹238,800 | ₹281,784 |
| | | **Total** | **100%** | **₹2,388,000** | **₹2,817,840** |

Each is paid against a checkable exit criterion, not a date — a milestone that
cannot be failed is not a milestone. M2 is asserted by a test suite run in front
of the client; M5 by an automated check that every source path resolves to
exactly one row.

**Cash flow.** Peak negative position **−₹6.16 lakh at week 8, 25.8% of contract
value**, on a four-week payment lag. That is working capital, not margin, and it
is the number a small supplier should look at first. The last milestone's cash
arrives after the assignment closes.

**Resourcing** is front-loaded on purpose: three people for weeks 5–8 on the
remediation sprint, settling to ~1.5 FTE through the inventory, tapering to
closeout. Peak 2.77 FTE, average 1.49.
...
| [`model/schedule.json`](model/schedule.json) | Start/end weeks, milestones, exit criteria, commercial terms |
| [`model/build_schedule.py`](model/build_schedule.py) | Builds the timeline and payment workbook from the model |
